...
3-2-1.실시간 처리 활성 옵션을 통해 해당 썸네일 옵션에 실시간 처리 여부를 결정할 수 있습니다. 또한 Gif 애니메이션 효과 유지 여부를 선택할 수 있습니다.  
3-2-2.이미지 생성 품질을 설정합니다. 단, 손실 압축 방식 이미지(예: JPEG 등)에 적용 가능합니다. (권장값 80%)  
3-2-3.이미지 생성 포맷을 선택합니다. (기본값은 원본 포맷유지)  
3-2-4."이미지 메타정보 유지"와 "Orientation 정보를 기준으로 이미지 회전" 옵션을 서비스에 맞게 체크/체크해제 합니다.  
...
3-4.[그림8]에 썸네일 오퍼레이션 시나리오는 썸네일 크기, 흑백필터, 크롭(Rectangle, Circle, Slice) 옵션을 제공합니다.  
